 ...
 *                               TELEMETRY:
 *
 * classic_mlnn_xopter uses WiFi for telemetry.
 ...
 *      Till now, MPU6050 is the only gyroscope admitted on classic_mlnn_xopter.
 */
 ...
 * AUTOMATIC PID:
 * 
 *      Auto-tune PID allow classic_mlnn_xopter to adjust on run the PID parameters to best fit the environmental changes.
 ...
 *      Leave OFF if you don't have any pressure sensor installed on classic_mlnn_xopter.
 ...
 *  GPS:
 *
 *      GPS sensor is useful to recover the latitude and longitude position of classic_mlnn_xopter.
 *      GPSs talk with classic_mlnn_xopter using a serial communication (TX/RX).
 ...
 *      Otherwise, this feature may signal a battery low level while, when classic_mlnn_xopter is still at ground, the battery level is above
